...
names.each { |name| puts "Goodbye #{name}!" }

puts

# ranges
(5..10).each.with_index do |num, idx|
...
# const resusable = () => //do stuff
# dogs.forEach(resusable)
...
# lambdas
# "named blocks"
do_thing = lambda { |dog, cat| return dog } # lambda keyword
...
def my_method(&block)
  block.call if block # .call to invoke the block
end
my_method &say_something # w/o parens
my_method(&say_something) # w/ parens

def my_method_two
  yield # .call to invoke the block
end
